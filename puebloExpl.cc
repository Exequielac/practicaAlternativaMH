...
class puebloExplorador{
  ...
  public:
    ...
    puebloExplorador(int dim, double limitInf, double limitSup, mt19937 &gen){
      ...
      uniform_int_distribution<> distPaciencia(20 * dim, 40 * dim);
...
void evolucionaPueblos(int dim, double limitInf, double limitSup, mt19937 &gen){
  ...
  {
    // En primer lugar, se mira entre posibles enfrentamientos de pueblos
    ...
    // En segundo lugar, se crea un vector normal aleatorio en esta iteración
    ...
    for(int i = 0; i < mundo.size() && evalsAct < maxEvals; i++){
      ...
      if (nuevoValor < mundo[i].getValorSol() || mundo[i].getNumItersNoMejora() == mundo[i].getPacienciaPueblo()){
  ...
  for(int i = 0; i < dims.size(); i++)
    for(int j = 0; j < seeds.size(); j++){ // Se ejecuta 10 veces, hay 10 semillas
      ...
      for(int funcid = 1; funcid <= 30; funcid++){
        cec17_init("puebloExpl", funcid, dims[i]);
